...
class CosineScheduler:
    ...
    def __init__(
        ...
        ks = torch.arange(0, self.k_train + 1, dtype=torch.float32)
        t = ks / float(self.k_train)
    ...
    def add_noise(
        ...
        ab = self.alpha_bar(k).view(-1, 1, 1)
    ...
    def ddim_step(
        ...
        ab_k = self.alpha_bar(k).view(-1, 1, 1)
        ab_prev = self.alpha_bar(k_prev).view(-1, 1, 1)

        sqrt_ab_k = torch.sqrt(ab_k)
        sqrt_one_minus_ab_k = torch.sqrt(torch.clamp(1.0 - ab_k, min=1e-6))

        x0_pred = (xt - sqrt_one_minus_ab_k * eps_pred) / torch.clamp(sqrt_ab_k, min=1e-6)
        x0_pred = torch.clamp(x0_pred, -1.0, 1.0)

        xt_prev = torch.sqrt(ab_prev) * x0_pred + torch.sqrt(
            torch.clamp(1.0 - ab_prev, min=1e-6)
        ) * eps_pred
        return xt_prev

    @torch.no_grad()
    def sample(self, net, obs: torch.Tensor, ta: int) -> torch.Tensor:
        """Full DDIM denoising loop, returns action sequence A^0."""
        batch_size = obs.shape[0]
        device = obs.device
        xt = torch.randn(batch_size, ta, 2, device=device)
        ...
        for i in range(len(self.infer_steps) - 1):
            ...
            k_prev_tensor = torch.full(
                (batch_size,), k_prev, dtype=torch.long, device=device
            )
